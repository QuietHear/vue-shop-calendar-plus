...
/*
 * @LastEditors: aFei
 * @LastEditTime: 2022-11-23 17:29:20
*/
<template>
...
export default {
  ...
  setup(props, { attrs, slots, emit, expose }) {
      ...
      for (let i = 0; i < beforeNum; i++) {
        let obj = dealOneDay(
          `${beforeYear}-${beforeMonth}-${
            checkDays(beforeYear, beforeMonth) - i
          }`
        );
    ...
    const topMiddle = computed(() => {
      return `${selectDay.value.year}${props.i18n ? "-" : "年"}${
        selectDay.value.month > 9 ? "" : "0"
